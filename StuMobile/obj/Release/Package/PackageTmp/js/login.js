$(function () {
    ...
    $(".obtain").on("click", function () {
        ...
        $.post("/service/Student/SendPhoneCodeBYLogin", { Phone: $(".username").val()}, function (rsp) {
            if (rsp.SuccessResponse) {
                $('.phoneBack').val(rsp.ReturnData.Data.Code)
                setCodeTime(rsp.ReturnData.Data.Seconds)
                layer.open({ content: rsp.Message, type: 0, time: 0.5 });
            }
            else {
                setCodeTime(rsp.ReturnData.Data.Seconds)
