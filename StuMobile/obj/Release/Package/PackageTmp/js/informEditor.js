...
function GetStudentInfo() {
    return new Promise(function (resolve, reject) {
        $.ajax({
            url: '/service/Student/GetStudentInfo',
            success: function (res) {
                if (res.SuccessResponse) {
                    resolve(res)
                }
...
function secondChange(picker, selectedIndex) {
    ...
    function GetEducation(Education) {
        picker2=$.get("/service/HDictionary/GetHDictionaryList", { Valid: 1, ColumnName: 'Education_ID' }, function (res) {
            if (res.SuccessResponse) {
                var Education = [];
...
//头像上传
    ...
    $("button.saveUp.informationSave").on("click", function () {
        ...
        $.post("/service/Student/EditStudentInfoByStuId", data, function (rsp) {
