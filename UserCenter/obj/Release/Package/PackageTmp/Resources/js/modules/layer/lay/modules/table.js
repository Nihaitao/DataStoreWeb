    ...
    function(exports) {
        ...
        var $ = layui.$,
            ...
            TPL_HEADER = function(options) {
                ...
                return [
                    ...
                    '{{# if(item2.fixed){ right = true; } }}', function() {
            ...
            ,
            TPL_MAIN = [
                '<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" style="margin-bottom:30px; {{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">',
                '{{# var fixed, right; }}', '<div class="layui-table-header">', TPL_HEADER(), '</div>',
                '<div class="layui-table-body layui-table-main">', TPL_BODY, '</div>',
                '{{# if(fixed && fixed !== "right"){ }}', '<div class="layui-table-fixed layui-table-fixed-l">',
                '<div class="layui-table-header">', TPL_HEADER({ fixed: true }), '</div>',
                '<div class="layui-table-body">',
                TPL_BODY, '</div>', '</div>', '{{# }; }}', '{{# if(right){ }}',
                '<div class="layui-table-fixed layui-table-fixed-r">', '<div class="layui-table-header">',
                TPL_HEADER({ fixed: 'right' }), '<div class="layui-table-mend"></div>', '</div>',
                '<div class="layui-table-body">', TPL_BODY, '</div>', '</div>', '{{# }; }}', '{{# if(d.data.page){ }}',
                '<div class="layui-table-tool">',
                '<div class="layui-inline layui-table-page" id="layui-table-page{{d.index}}"></div>', '</div>',
                '{{# } }}',
            ...
            if (options.url) { //Ajax请求
                $.ajax({
                    ...
                    success: function(res) {
                        res.code = res.SuccessResponse ? 0 : 100;
                        res.msg = res.Message;
                        if (res.code != 0) {
                            return layer.msg(res.msg == null ? "接口返回失败！" : res.msg);
                        }
                        res.count = res.ReturnData.TotalCount;
                        res.data = res.ReturnData.Data;
        ...
        Class.prototype.renderData = function(res, curr, count, sort) {
                        ...
                        function(i1, item1) {
                            ...
                            that.eachCols(function(i3, item3) {
                                ...
                                var td = [
                                    ...
                                    function() {
                                        var attr = [];
                                        if (typeof item3.edit == 'boolean') {
                                            if (item3.edit) attr.push(' data-edit="true"'); //是否允许单元格编辑
                                        } else if (typeof item3.edit == 'function') {
                                            if (item3.edit.call(dataItem, dataItem)) {
                                                attr.push(' data-edit="true"');
                                            }
                                        }
                                        if (item3.align)
                                            attr.push(' data-off="click" align="' + item3.align + '"'); //对齐方式
                                    ...
                                    function() {
                                        ...
                                        if (item3.fixed === 'right' && item3.toolbar) {
                                            if (typeof (item3.toolbar) == 'function') {
                                                return item3.toolbar.call(item1, item1);
                                            } else {
                                                return $(item3.toolbar).html();
                                            }
                                        }
                                        if (typeof (item3.templet) == 'function') {
                                            return item3.templet.call(item1, item1);
                                        }
                                        return item3.templet
                                            ? laytpl($(item3.templet).html() || String(content)).render(item1)
                                            : content;
                                    }(), '</div></td>'
                                ].join('');
            ...
            if (options.page) {
                ...
                that.layTool.css("width", (document.body.clientWidth - 20) + "px").css("left", "0px")
                    .css("left", "10px").css("border-width", "1px") //2017-09-11赵小龙修改
            }
        };

        //数据排序
        Class.prototype.sort = function(th, type, pull) {
            var that = this, field, config = that.config, thisData = table.cache[that.key];
        ...
        Class.prototype.setCheckData = function(index, checked) {
            var that = this, config = that.config, thisData = table.cache[that.key];
        ...
        Class.prototype.events = function() {
            ...
            th.on('click',
                ...
                }).find(ELEM_SORT + ' .layui-edge ').on('click',
                function(e) {
            ...
            //单元格事件
            that.layBody.on('click',
                'td',
                function() {
                    var othis = $(this), field = othis.data('field'), elemCell = othis.children(ELEM_CELL);
                        ...
                        that.tipsIndex = layer.tips([
                            ...
                            elemCell[0],
                            {
                                ...
                                success: function(layero, index) {
                                    layero.find('.layui-table-tips-c').on('click',
                                        function() {
                                            layer.close(index);
                                        });
                                }
        ...
        table.checkStatus = function(id) {
            var nums = 0, arr = [], data = table.cache[id];
            if (!data) return {};
            //计算全选个数
            layui.each(data,
                function(i, item) {
